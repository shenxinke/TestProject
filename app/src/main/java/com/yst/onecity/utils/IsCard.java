...
public class IsCard {
    ...
    public boolean verifyLength(String code) {
        ...
        if (length == Const.INTEGER_15 || length == Const.INTEGER_18) {
            return true;
        } else {
    ...
    public boolean verifyBirthdayCode(String code) {
        ...
        //2月的情况
            ...
            //闰月的情况
            boolean flag = (year % Const.INTEGER_4 == 0 && year % Const.INTEGER_100 != 0) || (year % Const.INTEGER_400 == 0);
            if (flag) {
                if (Integer.valueOf(dayCode) > Const.INTEGER_29 || Integer.valueOf(dayCode) < 1) {
            ...
            //非闰月的情况
                ...
                if (Integer.valueOf(dayCode) > Const.INTEGER_28 || Integer.valueOf(dayCode) < 1) {
    ...
    public boolean containsAllNumber(String code) {
        String str = "";
        if (code.length() == Const.INTEGER_15) {
            str = code.substring(0, 15);
        } else if (code.length() == Const.INTEGER_18) {
    ...
    public boolean verify(String idcard) {
        ...
        //如果是15位的就转成18位的身份证
        String eifhteencard = "";
        if (idcard.length() == Const.INTEGER_15) {
    ...
    public String getVerify(String eightcardid) {
        ...
        if (eightcardid.length() == Const.INTEGER_18) {
            eightcardid = eightcardid.substring(0, 17);
        }

        if (eightcardid.length() == Const.INTEGER_17) {
            int sum = 0;
            for (int i = 0; i < Const.INTEGER_17; i++) {
