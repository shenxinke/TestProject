...
public class HostLoginActivity extends Activity {
    ...
    private int loginWay = Constants.SMS_LOGIN;
    final static int STR_ACCOUNT_LOGIN_REQUEST = 10000;
    final static int SMS_REG_REQUEST = 10001;
    //private int loginWay = Constants.SMS_LOGIN | Constants.QQ_LOGIN | Constants.WX_LOGIN;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(MyResource.getIdByName(getApplication(), "layout", "tencent_tls_ui_activity_host_login"));
        ...
        // 短信登录
        if ((loginWay & Constants.SMS_LOGIN) != 0) {
            initSmsService();
/*            smsContentObserver = new SmsContentObserver(new Handler(),
                    this,
                    (EditText) findViewById(MyResource.getIdByName(getApplication(), "id", "checkCode_hostLogin")),
...
///        tlsService.initTlsSdk(HostLoginActivity.this);
        tlsService.initSmsLoginService(this,
                (EditText) findViewById(MyResource.getIdByName(getApplication(), "id", "selectCountryCode_hostLogin")),
                (EditText) findViewById(MyResource.getIdByName(getApplication(), "id", "phoneNumber_hostLogin")),
                (EditText) findViewById(MyResource.getIdByName(getApplication(), "id", "checkCode_hostLogin")),
                (Button) findViewById(MyResource.getIdByName(getApplication(), "id", "btn_requireCheckCode_hostLogin")),
                (Button) findViewById(MyResource.getIdByName(getApplication(), "id", "btn_hostLogin"))
        );

///       initTLSLogin();

        // 设置点击"注册新用户"事件
        findViewById(MyResource.getIdByName(getApplication(), "id", "hostRegisterNewUser"))
        ...
        // 用户名登录
        findViewById(MyResource.getIdByName(getApplication(), "id", "accountLogin"))
                .setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View view) {
            ...
            public void run() {
                ...
                if (userInfo != null) {
                    EditText editText = (EditText) HostLoginActivity.this
                            .findViewById(MyResource.getIdByName(getApplication(), "id", "phoneNumber_hostLogin"));
