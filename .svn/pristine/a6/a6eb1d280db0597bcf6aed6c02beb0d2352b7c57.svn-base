...
public class ShoppingMallAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    private LayoutInflater mLayoutInflater;
    private List<ShoppingMallBean.ContentBean> mData = new ArrayList<>();
    private int mtype;

    private ShoppingMallListener shoppingMallListener;

    public void setShoppingMallListener(ShoppingMallListener shoppingMallListener) {
    ...
    public void addList(List<ShoppingMallBean.ContentBean> data, int type) {
        if (data != null) {
            mData = data;
            mtype = type;
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
            return new ShoppingMallHolder(mLayoutInflater.inflate(R.layout.shopping_mall_item, parent, false));
    }
    ...
    public void setAdapterListener(final int position, final RecyclerView.ViewHolder holder) {
        if (holder instanceof ShoppingMallHolder) {
            //按钮点击事件
            if (shoppingMallListener != null) {
                ((ShoppingMallHolder) holder).imgShoppingCard.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View view) {
                        String state = String.valueOf(((ShoppingMallHolder) holder).id);
                        shoppingMallListener.btShoppingCart(position, state);
                    }
                });
                //故事点击事件
                ((ShoppingMallHolder) holder).tvStory.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View view) {
                        String state = String.valueOf(((ShoppingMallHolder) holder).id);
            ...
            //条目点击事件
            ((ShoppingMallHolder) holder).clParent.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View view) {
