...
public class WheelView extends ScrollView {
     ...
     */
    public static final int TEXT_COLOR_FOCUS = 0XFFFFCE1C;
    /**
     ...
     */
    public static final int LINE_COLOR = 0XFFFFCE1C;
    /**
    ...
    private void init(Context context) {
        ...
        if (Build.VERSION.SDK_INT >= Const.INTEGER_9) {
    ...
    private void refreshItemView(int y) {
        ...
        if (remainder == 0) {
            ...
            if (remainder > itemHeight / Const.INTEGER_2) {
    ...
    public void setBackgroundDrawable(Drawable background) {
        if (viewWidth == 0) {
            viewWidth = WindowUtils.getScreenWidth((Activity) context);
        }
    ...
    private void setWheelViewItems(List<String> list) {
        items.clear();
        items.addAll(list);
    ...
    public void setOffset(int offset) {
        if (offset < 1 || offset > Const.INTEGER_4) {
    ...
    public void setSelectedItem(String item) {
        for (int i = 0; i < items.size(); i++) {
            if (items.get(i).equals(item)) {
                //setWheelViewItems(List)时额外添加了offset个占位符到items里，需要忽略占位符所占的位
