 ...
 * <p>
 * This layout_item should be made the parent of the view that will be refreshed as a
 * result of the gesture and can only support one direct child. This view will
 * also be made the target of the gesture and will be forced to match both the
 * width and the height supplied in this layout_item. The SwipeRefreshLayout does not
...
public class SwipyRefreshLayout extends ViewGroup {
    ...
    private void startScaleUpAnimation(AnimationListener listener) {
        mCircleView.setVisibility(View.VISIBLE);
            if (android.os.Build.VERSION.SDK_INT >= Const.INTEGER_11) {
...
///        mProgress.setBackgroundColor(getResources().getColor(colorRes));
    ...
    /**
     * @return Whether it is possible for the child view of this layout_item to
     * scroll up. Override this if the child view is a custom view.
     */
    public boolean canChildScrollUp() {
        if (android.os.Build.VERSION.SDK_INT < Const.INTEGER_14) {
...
//    }
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        boolean flag = mReturningToStart && action == MotionEvent.ACTION_DOWN;
        if (flag) {
            mReturningToStart = false;
        }

        boolean bottomFlag = !isEnabled() || mReturningToStart || (!mBothDirection && canChildScrollDown()) || mRefreshing;
        boolean topFlag = !isEnabled() || mReturningToStart || (!mBothDirection && canChildScrollUp()) || mRefreshing;
        switch (mDirection) {
            case BOTTOM:
                if (bottomFlag) {
        ...
        final float xs = ev.getX();
        final float ys = ev.getY();
        switch (action) {
...
//                if (CartFragmentnew.isShopCartFragment && !CartFragmentnew.isCartRrefresh) {
//                    return false;
//                }
...
//                }
                ...
                final float deltaX = Math.abs(xs - mDownPosX);
                final float deltaY = Math.abs(ys - mDownPosY);
                if (deltaX > deltaY) {
                    return false;
                }
                ...
                if (mBothDirection) {
                    ...
                    boolean b = (mDirection == SwipyRefreshLayoutDirection.BOTTOM && canChildScrollDown())
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        try {
            ...
            MyLog.e(TAG, "An exception occured during SwipyRefreshLayout onTouchEvent " + e.toString());
        }
...
//        }
        mCurrentTargetOffsetTop = mCircleView.getTop();
        if (requiresUpdate && android.os.Build.VERSION.SDK_INT < Const.INTEGER_11) {
            invalidate();
        }
    }
