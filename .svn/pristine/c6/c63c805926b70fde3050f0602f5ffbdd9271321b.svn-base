...
import com.yst.onecity.bean.chatmodel.FriendFuture;
...
public class FriendManageMessageAdapter extends ArrayAdapter<FriendFuture> {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        viewHolder.status.setTextColor(ContextCompat.getColor(getContext(), R.color.text_gray1));
        switch (data.getType()){
            case TIM_FUTURE_FRIEND_PENDENCY_IN_TYPE:
                viewHolder.status.setText(res.getString(R.string.newfri_agree));
                viewHolder.status.setTextColor(ContextCompat.getColor(getContext(), R.color.text_blue1));
