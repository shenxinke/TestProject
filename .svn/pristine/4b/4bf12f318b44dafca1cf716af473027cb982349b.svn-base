        ...
        OkHttpUtils.post().url(Constants.CONSULT_DETAIL)
            ...
            public void onResponse(ConsultDetailBean response, int id) {
                if (response.getCode() == 1) {
                    ...
                    mTxtShareViews.setText(String.valueOf("浏览"+bean.getRead_num()));
                    mReleaseUserId = String.valueOf(bean.getUser_id());
                    mReleaseUserType = bean.getUser_type();
                    Glide.with(getApplicationContext()).load(ConstUtils.matchingImageUrl(bean.getMemberHeadImg())).placeholder(R.mipmap.head_2).error(R.mipmap.head_2).into(mIvShareHead);
                    ...
                    if (userType == 0) {
                        Glide.with(getApplicationContext()).load(ConstUtils.matchingImageUrl(bean.getHeadImg())).transform(new GlideCircleTransform(getApplicationContext())).placeholder(R.mipmap.head_2).error(R.mipmap.head_2).into(mIvPublishHead);
                        mTxtShareName.setText(bean.getName());
                    }else if (userType == Const.INTEGER_3) {
                        Glide.with(getApplicationContext()).load(R.drawable.icon_logo).placeholder(R.mipmap.head_2).error(R.drawable.icon_logo).into(mIvPublishHead);
        ...
        adapter = new AbstractCommonAdapter<ConsultCommentBean.ContentBean.InfoBean>(this, datas, R.layout.item_comment) {
            ...
            public void convert(CommonViewHolder holder, int position, ConsultCommentBean.ContentBean.InfoBean item) {
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR1) {
                    if (!NewDetailActivity.this.isDestroyed()) {
                        Glide.with(NewDetailActivity.this).load(ConstUtils.matchingImageUrl(item.getHeadImg())).error(R.mipmap.head_2).into((RoundedImageView) holder.getView(R.id.head));
                    } else {
