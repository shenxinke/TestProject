...
public class SwipeMenuLayout extends FrameLayout {
	...
	private int minFling = dp2px(15);
	private int maxVelocityx = -dp2px(500);
	...
	private void init(int mid) {
		...
		mGestureListener = new SimpleOnGestureListener() {
			...
			public boolean onFling(MotionEvent e1, MotionEvent e2,
				...
				// TODO
				if ((e1.getX() - e2.getX()) > minFling
						&& velocityX < maxVelocityx) {
					isFling = true;
				}
				/// MyLog.i("byz", maxVelocityx + ", velocityX = " + velocityX);
		...
		if (mContentView.getId() < Integer.parseInt(Const.STR1)) {
	...
	public boolean onSwipe(MotionEvent event) {
		...
		case MotionEvent.ACTION_MOVE:
			/// MyLog.i("byz", "downX = " + mDownX + ", moveX = " + event.getX());
		...
		case MotionEvent.ACTION_UP:
			if (isFling || (mDownX - event.getX()) > (mMenuView.getWidth() / Const.INTEGER_2)) {
				// open
				smoothOpenMenu();
			} else {
				// close
	...
	public void setMenuHeight(int measuredHeight) {
		MyLog.i("byz", "pos = " + position + ", height = " + measuredHeight);
